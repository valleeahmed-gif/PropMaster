...
export function ToastContainer() {
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[100] flex flex-col gap-2 w-[calc(100vw-2rem)] max-w-sm pointer-events-none">
      {toasts.map(t => (
        <div key={t.id} className={`flex items-center gap-3 px-4 py-3 rounded-xl border shadow-lg ${bg[t.type]} toast-enter pointer-events-auto`}>
          {icons[t.type]}
          <span className="text-sm text-gray-800 flex-1">{t.message}</span>
          <button onClick={() => dismissToast(t.id)} className="p-0.5 rounded hover:opacity-70">
