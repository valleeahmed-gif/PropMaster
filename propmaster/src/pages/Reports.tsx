...
function ProfitLossReport() {
  ...
  const pnlData = useMemo(() => {
    ...
    return props.map(prop => {
      ...
      // ── Property costs for this period ────────────────────
      ...
      // Recoverable = utility costs you collect from tenant BUT must pay to COJ.
      // This money passes through you — it is NOT profit.
      const recoverableCosts = allBreakdowns
        .filter(b => b.isRecoverable)
        .reduce((s, b) => s + b.amount, 0);

      // Non-recoverable = levy, rates, insurance — your actual expenses.
      const nonRecoverableCosts = allBreakdowns
        .filter(b => !b.isRecoverable)
        .reduce((s, b) => s + b.amount, 0);

      // ── Correct P&L calculation ───────────────────────────
      // What you actually KEEP = total collected minus the pass-through utility money
      // that must go to COJ, minus your own non-recoverable costs.
      //
      // totalCollected = rent + recoverable utilities (from tenant)
      // recoverableCosts = utilities you must pay to COJ (pass-through — not yours)
      // nonRecoverableCosts = levy/rates you pay yourself (real expense)
      //
      // netProfit = totalCollected - recoverableCosts - nonRecoverableCosts
      //           = (rent component) - nonRecoverableCosts
      const netProfit = totalCollected - recoverableCosts - nonRecoverableCosts;

      // Rent income = what you keep before expenses (collected minus pass-through)
      const rentIncome = totalCollected - recoverableCosts;

      return {
        property: prop,
        totalCollected,       // everything that came in
        recoverableCosts,     // utilities collected but owed to COJ (pass-through)
        nonRecoverableCosts,  // levy/rates — your real cost
        rentIncome,           // collected minus pass-through = the rent portion
        netProfit,            // what you actually keep
        paymentCount: propPayments.length,
        payments: propPayments,
      };
    });
  }, [selectedProperty, fromMonth, fromYear, toMonth, toYear, payments, propertyCosts, utilityBreakdowns]);
      ...
      <div className="card p-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 items-end">
          ...
          <div>
            ...
            <select className="input" value={fromYear} onChange={e => setFromYear(Number(e.target.value))}>
                        ...
                        <div className="divide-y divide-surface-100">
                          ...
                          {row.nonRecoverableCosts > 0 && (
                            <div className="flex justify-between items-center px-4 py-2.5 bg-red-50">
                              <div>
                                <span className="text-xs text-red-800">Less: your own costs</span>
                                <p className="text-xs text-red-600">Levy, rates, insurance</p>
                              </div>
                              <span className="text-xs font-semibold text-red-700 flex-shrink-0 ml-3">− {formatCurrency(row.nonRecoverableCosts)}</span>
                            </div>
                          )}
