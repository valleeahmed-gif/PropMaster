...
function InvoiceRow({ inv, onMarkSent, onRecordPayment, onEdit, onDelete }: {
  ...
  const linkedPayments = payments.filter(p => p.invoiceId === inv.id && p.status === 'verified');
        ...
        <div className="flex-1 min-w-0">
          ...
          {inv.status === 'paid' && linkedPayments.length > 0 && (
            <p className="text-xs text-green-600 font-medium mt-0.5">
              ✓ Paid {formatDate(linkedPayments[linkedPayments.length - 1].paymentDate)}
            </p>
          )}
          {inv.status === 'partial' && totalPaid > 0 && (
            <p className="text-xs text-amber-700 font-medium mt-0.5">
              {formatCurrency(totalPaid)} paid · {formatCurrency(balance)} still outstanding
            </p>
          )}
          {inv.status === 'overdue' && (
            <p className="text-xs text-red-600 font-medium mt-0.5">
              Overdue — {balance > 0 ? `${formatCurrency(balance)} outstanding` : 'awaiting payment'}
            </p>
          )}
        </div>
          ...
          {linkedPayments.length > 0 && (
            <>
              ...
              <div className="space-y-1.5">
                ...
                {totalPaid > 0 && (
                  <div className="flex justify-between text-xs pt-1.5 border-t border-surface-200">
                ...
                {balance > 0 && inv.status !== 'paid' && (
                  <div className="flex justify-between text-xs">
                    <span className={inv.status === 'overdue' ? 'text-red-600 font-semibold' : 'text-amber-700 font-semibold'}>
                      {inv.status === 'overdue' ? 'Overdue balance' : 'Outstanding balance'}
                    </span>
                    <span className={`font-bold ${inv.status === 'overdue' ? 'text-red-700' : 'text-amber-800'}`}>
